...
def handler(event: dict, context) -> dict:
    """Анализ сна через OpenRouter с интерпретацией по Юнгу и Фрейду. v5"""

    api_key = os.environ.get('OPENROUTER_API_KEY', '')
    print(f"KEY_DEBUG: len={len(api_key)} prefix={api_key[:12] if len(api_key) > 12 else api_key}")
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            'X-Title': 'Morpheus Dream Analyzer',
        },
        method='POST'
    )
